...
class AccountMove(models.Model):
    ...
    wh_iva = fields.Boolean('¿Ya se ha retenido esta factura con el IVA?', copy=False ,help="Los movimientos de la cuenta de la factura han sido retenidos con movimientos de cuenta de los pagos.")
    wh_iva_id = fields.Many2one('account.wh.iva', string='Documento de Retención de IVA', compute='_compute_wh_iva_id', store=True, help="Este es el documento de retención de IVA donde en esta factura está siendo retenida.")
    vat_apply = fields.Boolean(string='Excluir este documento de la retención del IVA', states={'draft': [('readonly', False)]}, help="Esta selección indica si generar la factura documento de retención")
    consolidate_vat_wh = fields.Boolean(string='Group wh doc', readonly=True, states={'draft': [('readonly', False)]}, default=False, help="Esta selección indica agrupar esta factura en documento de retención")


    @api.depends('wh_iva_id.wh_lines')
    def _compute_wh_iva_id(self):
        for record in self:
            lines = self.env['account.wh.iva.line'].search([('invoice_id', '=', record.id)])
    ...
    def action_post(self):
        ...
        if self:
            ...
            #_withholdable_tax() Verifica si se añadió diario de retencion de IVA en la factura
            resul = self._withholdable_tax()
            #Ahora procede a verificar si existe monto de impuesto para poder  hacer los cálculos
            for inv in self.line_ids:
                if len(self.line_ids.tax_ids) == 1:
                    for tax in inv.tax_ids:
                        if tax.amount == 0:
                            monto_tax = 2000  #¿No sería más estético que "monto_tax" fuera booleano?
        # print("Descripción")
        # print("Id del cliente/ proveedor (.partner_id): ", self.partner_id)
        # print("¿Es el cliente/proveedor agente de retención? (.partner_id.wh_iva_agent)", self.partner_id.wh_iva_agent)
        # print("Id de My Company (solo .company_id): ",self.company_id)
        # print("Id de My Company (con .partner_id): ", self.company_id.partner_id)
        # print("¿Es My Company agente de retención?", self.company_id.partner_id.wh_iva_agent)
        # print("OKI")


        # --------------------------------------------Retención a nivel de compras:--------------------------------------------------------------------------------
        # Aquí entran los casos de:
        # 1) My Company SI es agente de retención y NO tiene tasa, el proveedor NO es agente de retención y SI tiene tasa
        # 2) My Company SI es agente de retención y SI tiene tasa, el proveedor NO es agente de retención y SI tiene tasa
        # 3) My Company SI es agente de retención y NO tiene tasa, el proveedor SI es agente de retención y SI tiene tasa
        # 4) My Company SI es agente de retención y SI tiene tasa, el proveedor SI es agente de retención y SI tiene tasa

        

        # Tiene que haber un condicional previo porque:
        # puede que se trate de una venta en la que el cliente NO es agente de retención pero My Company SI
        # entonces en ese caso entraría y haría una retención al cliente como si fuera un proveedor
        # cuando no debería hacerla

        if self.line_ids.tax_ids.type_tax_use == 'purchase':

            if self.company_id.partner_id.wh_iva_agent and resul and monto_tax == 0 and self.partner_id.wh_iva_rate != 0.0 and self.company_id.partner_id.vat_subjected:    
                """***********************RETENCION TIPO COMPRA**********************"""
                if self.state == 'posted':
                    for ilids in self.invoice_line_ids:
                        self.check_document_date()
                        apply = self.check_wh_apply() 

                        """ La función check_wh_apply() vuele a verificar si se añadió 
                            diario de retencion de IVA en la factura (es decir, llama 
                            a _withholdable_tax() cuando esta ya fue llamada previamente 
                            al inicio del action_post() y fue asignada a la variable result),
                            verifica el tipo de factura y que el proveedor sea agente de retencion 
                            pero yo le comenté esa últmia funcionalidad, es decir que solo llama a 
                            _withholdable_tax() y luego verifica el tipo de factura"""

                        if apply == True:
                            self.check_withholdable()  # Retorna un booleano si es tipo de factura es 'in_invoice' ó 'out_invoice'
                            self.action_wh_iva_supervisor()
                            self.action_wh_iva_create()
                return self

        # --------------------------------------------Retención a nivel de ventas:--------------------------------------------------------------------------------

        # Aquí entran los casos de:
        # 1) El clietne SI es agente de retención y NO tiene tasa, My Company NO es agente de retención y SI tiene tasa
        # 2) El clietne SI es agente de retención y NO tiene tasa, My Company SI es agente de retención y SI tiene tasa
        # 3) El clietne SI es agente de retención y SI tiene tasa, My Company NO es agente de retención y SI tiene tasa
        # 4) El clietne SI es agente de retención y SI tiene tasa, My Company SI es agente de retención y SI tiene tasa
        
        elif self.line_ids.tax_ids.type_tax_use == 'sale':
    
            if self.partner_id.wh_iva_agent and resul and monto_tax == 0 and self.company_id.partner_id.wh_iva_rate != 0.0 and self.partner_id.vat_subjected:    
                """***********************RETENCION TIPO VENTA**********************"""
                if self.state == 'posted':
                    for ilids in self.invoice_line_ids:
                        self.check_document_date()
    ...
    def check_document_date(self):
        #check that the invoice in open state have the document date defined. @return True or raise an orm exception.
        for inv_brw in self:
            if (inv_brw.type in ('in_invoice', 'in_refund', 'out_invoice', 'out_refund') and inv_brw.state == 'posted' and not inv_brw.date):
                raise exceptions.except_orm( _('Advertencia'), _('La fecha del documento no puede estar vacía cuando la factura es en estado publicado.'))
        return True


    def check_invoice_dates(self):
        # check that the date document is less or equal than the date invoice. @return True or raise and osv exception.
        for inv_brw in self:
            if (inv_brw.type in ('in_invoice', 'in_refund', 'out_invoice', 'out_refund') and inv_brw.date and not inv_brw.date <= inv_brw.invoice_date):
                raise exceptions.except_orm( _('Warning'), _('The document date must be less or equal than the invoice date.'))
        return True
    ...
    def wh_iva_line_create(self):
        ...
        values = {}
        for inv_brw in self:   # self está en account.move
            wh_iva_rate = (
                inv_brw.type in ('in_invoice', 'in_refund', 'out_refund', 'out_invoice') and
                partner._find_accounting_partner(inv_brw.partner_id).wh_iva_rate or
                partner._find_accounting_partner(inv_brw.company_id.partner_id).wh_iva_rate)
        ...
        for inv in self:
            if inv.wh_iva_id:
                if inv.wh_iva_id.state == 'draft':
                    ...
                    raise exceptions.except_orm(
                        _('Advertencia!'),
                        _('Ya tiene un documento de retención asociado a su factura, pero este documento de retención no está en cancelar estado.'))
            else:
                # Create Lines Data
                ret_id = {}
                ...
                if inv.company_id.consolidate_vat_wh and fortnight_wh_id:
                    ...
                    if not ret_id:
                        raise exceptions.except_orm(_('Error!'), _('No se puede encontrar el documento de retención'))
    ...
    def _withholding_partner(self):
        # Verifica que el tipo de factura esté en uno de los 4 estados
        for inv in self:
            if inv.type in ('in_invoice', 'in_refund', 'out_invoice', 'out_refund'):
            # and self.env['res.partner']._find_accounting_partner(inv.company_id.partner_id).wh_iva_agent:
                return True
        return False

    def _withholdable_tax(self):
        # Revisa si al momento de llenar la factura se le añadió el impuesto IVA
        is_withholdable = False         # self está en la tabla account.move
        for inv in self.line_ids:       # inv está recorriendo la tabla account.move.line  
            for tax in inv.tax_ids:     # line_ids es un campo relacional
                print("inv.tax_ids:: ",inv.tax_ids)
                if tax.type_tax == 'iva':
    ...
    def check_wh_apply(self):
        # verifica que se pueda hacer a la factura
        wh_apply = []
        for inv in self:
            # vat_apply y sin_cred son campos booleanos en donde el usuario puede indicar si no quiere que se le haga retención a esta factura
            if inv.vat_apply or inv.sin_cred:
                return False
            # Ahora va a volver a verificar si se añadió diario de retencion de IVA en la factura
            wh_apply.append(inv._withholdable_tax())
            # Ahora va a verificar que la factura esté en uno de los 4 estados
            wh_apply.append(inv._withholding_partner())
            # Retorna verdad si todo lo anterior es verdad (obviamente a excepción de los campos "inv.vat_apply" y "inv.sin_cred") 
        return all(wh_apply)
