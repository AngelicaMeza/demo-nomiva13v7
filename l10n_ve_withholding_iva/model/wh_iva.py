...
class AccountWhIvaLineTax(models.Model):
    ...
    def _get_base_amount(self):
        ...
        for record in self:
            ...
            if record.id_tax:
                busq = self.env['account.tax'].search(
                    [('id', '=', record.id_tax)])
...
class AccountWhIvaLine(models.Model):
    ...
    def load_taxes(self):
        ...
        for rec in self:
            if rec.invoice_id:
                ...
                retetion_values = self._get_tax_line_values(rec)            
                awilt.create(retetion_values)

        return True
    
    @api.model
    def _get_tax_line_values(self, wh_iva_line):
        res = []
        invoice = wh_iva_line.invoice_id
        for line in invoice.line_ids:
            if bool(line.tax_line_id) is False:
                continue
            ...
            if len(line.tax_ids) > 1:
                taxxx = line.tax_ids[0]
            else:
                taxxx = line.tax_line_id

            for tax in taxxx:
                taxd = tax.id
            
            res.append({
                'wh_vat_line_id': wh_iva_line.id,
                'id_tax': taxd,
                'move_id': invoice.id,
                'base': base_amount,
                'amount': tax_amount,
    ...
    # 	return invoice_currency.with_context(date=self.invoice_id.date).compute(amount, company_currency)
            ...
            if rec.create_date != False:
                ...
                if rec.invoice_id:
                    ...
                    if rec.invoice_id.type == 'in_refund':
                        ...
                        rec.amount_tax_ret = sum(
                            l.amount_ret for l in rec.tax_line)
...
class AccountWhIva(models.Model):
    ...
    def _get_journal(self):
        ...
        res = []
        partner_sale = self.env['account.journal'].search(
            [('type', '=', 'sale'), ('code', '=', 'RIV')])

        partner_purchase = self.env['account.journal'].search(
            [('type', '=', 'purchase'), ('code', '=', 'RIC')])
    ...
    def get_reconciled_move(self):
        ...
        for a in obj_move_line:
            nombre = a.move_name
        if nombre:
            obj_move_name = self.env['account.move.line'].search(
                [('move_name', '=', nombre)])
        ...
        for ret in self:
            if ret.state == 'done':
                for ret_line in ret.wh_lines:
                    if ret_line.move_id:
                        ret_line.move_id._reverse_moves([{'date': fields.Date.today(), 'ref': _(
                            'Reversal of %s') % ret_line.move_id.name}], cancel=True)
    ...
    @api.model
    def _get_valid_wh(self, amount_ret, amount, wh_iva_rate, offset=0.5):
        ...
        return (amount_ret >= float((amount * (wh_iva_rate - offset) / 100.0)) and amount_ret <= float((amount * (wh_iva_rate + offset) / 100.0)))

    def check_wh_taxes(self):
        """ Check that are valid and that amount retention is not greater than amount
        """
        note = _(
            'Los impuestos en las siguientes facturas han sido mal calculados\n\n')
        error_msg = ''
        for record in self:
            wh_line_ids = []
    ...
    def _check_partner(self):
        ...
        for obj in self:
            if obj.type in ('out_invoice', 'out_refund'):
                ...
                print("WH_IVA LINEA 527")    
            else:
                if not partner._find_accounting_partner(
                        obj.company_id.partner_id).wh_iva_agent:
                    raise exceptions.ValidationError(
                        _('El socio debe estar reteniendo el iva agente.'))
                print("WH_IVA LINEA 533") 
                print("NO HAY MÁS CÓDIGO")   
            ...
            if ret.wh_lines:
                ...
                for line in ret.wh_lines:
                    if line.invoice_id.type in ['in_invoice', 'in_refund']:
                        name = ('COMP. RET. IVA ' + (ret.number if ret.number else str()) + ' Doc. ' + (
                            line.invoice_id.supplier_invoice_number if line.invoice_id.supplier_invoice_number else str()))
                    else:
                        name = ('COMP. RET. IVA ' + (ret.number if ret.number else str()) +
                                ' Doc. ' + (line.invoice_id.name if line.invoice_id.name else str()))
                    ...
                    wh_iva_tax_line = self.env['account.wh.iva.line.tax'].search(
                        [('wh_vat_line_id', '=', line.id)])
    ...
    def confirm_check(self):
        ...
        if (not self.check_wh_lines() or
            ...
            consulta = self.env['account.wh.iva'].search(
                [('name', '=', self.name), ('number_2', '!=', False)])
            if not consulta:
    ...
    def _get_default_company(self):
        res_company = self.env['res.company'].search(
            [('id', '=', self.wh_lines.invoice_id.company_id.id)])
        if not res_company:
    ...
    def update_ret_number(self):
        local_number = self.env['ir.sequence'].next_by_code(
            'purchase.account.wh.iva.sequence')
    ...
    def _amount_ret_all(self):
        ...
        for rec in self:
            ...
            if rec.create_date != False:
                if rec.wh_lines:
                    rec.total_tax_ret = sum(
                        l.amount_tax_ret for l in rec.wh_lines)
