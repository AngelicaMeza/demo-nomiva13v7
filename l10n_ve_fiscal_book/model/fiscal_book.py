...
class FiscalBook(models.Model):
   ...
   # company_id = fields.Many2one('res.company', 'Compañia', help='Company', )
    ...
    iva_ids = fields.One2many('fiscal.book.manual_iva', 'iva_ids', string='Retenciones de IVA Manuales')
    ...
    def update_book(self):
        ...
        for fb_brw in local_fb:
            ...
            #Esto elimina las lineas de facturas repetidas
            #No se sabe que hace que se generen duplicadas
    ...
    def reajuste_totales(self):
        ...
        for line in self.fbl_ids:
            if line.doc_type == 'N/C':
                base_amount -= line.vat_exempt + line.vat_general_base + line.vat_reduced_base + line.vat_additional_base
                tax_amount -= line.vat_general_tax + line.vat_reduced_tax + line.vat_additional_tax
            else:
                base_amount += line.vat_exempt + line.vat_general_base + line.vat_reduced_base + line.vat_additional_base
                tax_amount += line.vat_general_tax + line.vat_reduced_tax + line.vat_additional_tax
    ...
    def _get_wh_iva_line_ids(self, fb_id):
        ...
        awil_ids = []
        local_period = self.get_time_period(self.time_period)
        domain = [('wh_lines.invoice_id.date', '>=', local_period.get('dt_from')),
                  ('wh_lines.invoice_id.date', '<=', local_period.get('dt_to')),
                  ('type', 'in', awil_type),
                  ('state', '=', 'done')]
    ...
    # def get_invoice_name(invoice_name):
        
    #     invoice_num_name = invoice_name.name.split("/")
    #     if len(invoice_num_name) > 2:
    #         debit_codicional= invoice_num_name[len(invoice_num_name)-1]
    #         return debit_codicional
    #     else:
    #         debit_codicional="".join([str(s) for s in iwdl_brw.invoice_id.name if s.isdigit() ]) 
    #         return debit_codicional
    ...
    def update_book_lines(self, fb_id):
        ...
        if inv_iva:
            ...
            orphan_iwdl_ids = self._get_orphan_iwdl_ids(fb_id) # SEGUN YO, ESTO RETORNA UNA LISTA VACIA
            no_match_dt_iwdl_ids = self._get_no_match_date_iwdl_ids(fb_id) # SEGUN YO, ESTO RETORNA UNA LISTA VACIA
            iwdl_ids = orphan_iwdl_ids #  SEGUN YO, ESTO ASIGNA UNA LISTA VACIA
           
           
            for nm in no_match_dt_iwdl_ids:  # SEGUN YO, NO ENTRA EN ESE BUCLE
                iwdl_ids += nm
            ...
            for iwdl_brw in iwdl_ids:
                # if iwdl_brw.type == 'in_invoice':
                    ...
                    #doc_type = self.get_doc_type(inv_id=iwdl_brw.invoice_id.id)   LA COMENTÉ PERO CREO QUE DEBERIA DESCOMENTARLA


                    #if (doc_type == "N/D" or doc_type == "N/C"):
                        #if fb_brw.type == 'sale':
                    ...
                    #else:
                        #sign = 1
                    #if (doc_type == "N/D" or doc_type == "N/C") and local_inv_affected:
                        #boll = True
                    #else:
                        #boll = False
                    #if boll == True:
                    ...
                    doc_typee = self.get_doc_type(inv_id=iwdl_brw.invoice_id.id, iwdl_id=iwdl_brw.id)
                    if (self.get_doc_type(inv_id=iwdl_brw.invoice_id.id, iwdl_id=iwdl_brw.id) == 'N/C') : #NOTAS DE CREDITO
                        if iwdl_brw.invoice_id.reversed_entry_id: ## LIBRO DE COMPRA
                            if iwdl_brw.invoice_id.reversed_entry_id.supplier_invoice_number:
                                affected_invoice=iwdl_brw.invoice_id.reversed_entry_id.supplier_invoice_number
                            else:
                                affected_invoice=iwdl_brw.invoice_id.reversed_entry_id.nro_ctrl
                        elif iwdl_brw.invoice_id.credit_note_number:
                            affected_invoice=iwdl_brw.invoice_id.credit_note_number
                        else: ###LIBRO DE VENTAS

                            invoice_num_name = iwdl_brw.invoice_id.name.split("/")
                            if len(invoice_num_name) > 2:
                                affected_invoice= invoice_num_name[len(invoice_num_name)-1]
                            else:
                                affected_invoice="".join([str(s) for s in iwdl_brw.invoice_id.name if s.isdigit() ]) 
                            # affected_invoice=iwdl_brw.invoice_id.ref
                        
                        if iwdl_brw.invoice_id.supplier_invoice_number:
                            credit_codicional= iwdl_brw.invoice_id.supplier_invoice_number
                        else:
                            # invoice_num_name = "".join([str(s) for s in iwdl_brw.invoice_id.name if s.isdigit() ])  
                            invoice_num_name = iwdl_brw.invoice_id.name.split("/")
                            if len(invoice_num_name) > 2:
                                credit_codicional= invoice_num_name[len(invoice_num_name)-1]
                            else:
                                credit_codicional="".join([str(s) for s in iwdl_brw.invoice_id.name if s.isdigit() ])  
                        debit_codicional=''

                    elif iwdl_brw.invoice_id.debit_origin_id: ## NOTAS DE DEBITO
                        if iwdl_brw.invoice_id.supplier_invoice_number:## LIBRE DE COMPRA
                            debit_codicional= iwdl_brw.invoice_id.supplier_invoice_number
                        else:## libro de venta

                            # invoice_num_name = "".join([str(s) for s in  iwdl_brw.invoice_id.name if s.isdigit() ])  
                            invoice_num_name = iwdl_brw.invoice_id.name.split("/")
                            # debit_codicional= get_invoice_name(invoice_id=invoice_num_name)
                            if len(invoice_num_name) > 2:
                                debit_codicional= invoice_num_name[len(invoice_num_name)-1]
                            else:
                                debit_codicional="".join([str(s) for s in iwdl_brw.invoice_id.name if s.isdigit() ])  
                        ...
                        if iwdl_brw.invoice_id.debit_origin_id.supplier_invoice_number:
                            affected_invoice=iwdl_brw.invoice_id.debit_origin_id.supplier_invoice_number
                        else:
                            invoice_num_name = iwdl_brw.invoice_id.debit_origin_id.name.split("/")
                            if len(invoice_num_name) > 2:
                                affected_invoice= invoice_num_name[len(invoice_num_name)-1]
                            else:
                                affected_invoice="".join([str(s) for s in iwdl_brw.invoice_id.debit_origin_id.name if s.isdigit() ]) 
                            # affected_invoice=iwdl_brw.invoice_id.debit_origin_id.nro_ctrl
                        credit_codicional=""
                    ...
                    ### CONDICIONAL PARA NOTAS DE DEBITO MANUALES 
                    # if iwdl_brw.invoice_id.is_manual_debit_note:
                    #     if iwdl_brw.invoice_id.supplier_invoice_number:##### LIBRO DE COMPRA
                    #         debit_codicional = iwdl_brw.invoice_id.supplier_invoice_number
                    #     else:
                    #         invoice_num_name = iwdl_brw.invoice_id.name.split("/")
                    #         if len(invoice_num_name) > 2:
                    ...
                    #         else:
                    #             debit_codicional="".join([str(s) for s in iwdl_brw.invoice_id.name if s.isdigit() ]) 
                    #     affected_invoice=iwdl_brw.invoice_id.invoice_affected
                    #     credit_codicional=""
                    #     doc_typee = 'N/D'
                    ...
                    if iwdl_brw.invoice_id.supplier_invoice_number:
                        invoice_number = iwdl_brw.invoice_id.supplier_invoice_number
                    elif iwdl_brw.invoice_id.nro_ctrl:
                        invoice_num_ctrl = "".join([str(s) for s in iwdl_brw.invoice_id.name if s.isdigit() ]) 
                        invoice_number = invoice_num_ctrl
                    else:
                        invoice_number = iwdl_brw.invoice_id.ref

                    values = {'invoice_id' : iwdl_brw.invoice_id.id,
                                'iwdl_id': iwdl_brw.id,
                                'type': t_type,
                                'accounting_date': iwdl_brw.date_ret or False,  #'accounting_date': iwdl_brw.invoice_id.date or False,
                                ...
                                'debit_affected': debit_codicional, #debit_affected': local_inv_affected or False,
                                ...
                                #     False,
                                'credit_affected':  credit_codicional, #'credit_affected': local_inv_affected and cliente and proveedor or False,
                                #     inv_brw.parent_id and
                            ...
                            #   'affected_invoice':'',  #'affected_invoice': (doc_type == "N/D" or doc_type == "N/C") and local_inv_affected,
                                'affected_invoice': affected_invoice, #'affected_invoice': local_inv_affected,
                            
                                #'affected_invoice_date': fecha,
                                'wh_rate': iwdl_brw.wh_iva_rate,
                                'invoice_number': invoice_number if not debit_codicional and not credit_codicional  else ' ', # se agrega el campo numero de factura para las facturas fuera de periodo
                                'ctrl_number': iwdl_brw.invoice_id.nro_ctrl or "", # se agrega el campo numero de control para las facturas fuera de periodo
                                'void_form': self.get_t_type(doc_typee),
      ...
      #      if not self.browse(fb_id).iwdl_ids and iwdl_id == False:
        ...
        #        iwdl_brw = iwdl_id if iwdl_id and iwdl_id not in no_match_dt_iwdl_ids else False
                ...
                doc_type = self.get_doc_type(inv_id=inv_otro.id)   
                if doc_type == 'N/C' : ### NOTA DE CREDITO
                    
                    if inv_otro.reversed_entry_id: ## libro de compra
                        if inv_otro.reversed_entry_id.supplier_invoice_number:
                            affected_invoice=inv_otro.reversed_entry_id.supplier_invoice_number
                        else:
                            invoice_num_name = inv_otro.reversed_entry_id.name.split("/")
                            if len(invoice_num_name) > 2:
                                affected_invoice= invoice_num_name[len(invoice_num_name)-1]
                            else:
                                affected_invoice="".join([str(s) for s in inv_otro.reversed_entry_id.name if s.isdigit() ]) 
                                # affected_invoice=inv_otro.reversed_entry_id.nro_ctrl
                    elif inv_otro.credit_note_number:##  nota de credito manual
                        affected_invoice = inv_otro.credit_note_number
                    else: ###LIBRE DE VENTA
                        invoice_num_name = inv_otro.name.split("/")
                        if len(invoice_num_name) > 2:
                            affected_invoice= invoice_num_name[len(invoice_num_name)-1]
                        else:
                            affected_invoice="".join([str(s) for s in inv_otro.name if s.isdigit() ]) 
                        # affected_invoice=inv_otro.ref
                   
                   
                   
                   
                    if inv_otro.supplier_invoice_number:## LIBRO DE COMPRA
                        credit_codicional= inv_otro.supplier_invoice_number 
                    else:## libro de venta
                        invoice_num_name = inv_otro.name.split("/")
                        if len(invoice_num_name) > 2:
                            credit_codicional= invoice_num_name[len(invoice_num_name)-1]
                        else:
                            credit_codicional="".join([str(s) for s in inv_otro.name if s.isdigit() ])  
                    debit_codicional=""
                elif inv_otro.debit_origin_id: ##NOTA DE DEBITO

                    if inv_otro.supplier_invoice_number:## libro de compra
                        debit_codicional= inv_otro.supplier_invoice_number 
                    else:## libro de venta
                        invoice_num_name = inv_otro.name.split("/") 
                        ...
                        if len(invoice_num_name) > 2:
                            ...
                            debit_codicional="".join([str(s) for s in inv_otro.name if s.isdigit() ])  
                    credit_codicional=""
                    
                    if inv_otro.debit_origin_id.supplier_invoice_number:
                        affected_invoice=inv_otro.debit_origin_id.supplier_invoice_number
                    else:
                        invoice_num_name = inv_otro.debit_origin_id.name.split("/")
                        if len(invoice_num_name) > 2:
                            affected_invoice= invoice_num_name[len(invoice_num_name)-1]
                        else:
                            affected_invoice="".join([str(s) for s in inv_otro.debit_origin_id.name if s.isdigit() ]) 
                            # affected_invoice=iwdl_brw.invoice_id.debit_origin_id.nro_ctrl
                        # affected_invoice=inv_otro.debit_origin_id.nro_ctrl
                ...
                ####CONICIONAL PARA NOTAS DE DEBITO MANUALES 
                # if inv_otro.is_manual_debit_note:
                #     if inv_otro.supplier_invoice_number:##### LIBRO DE COMPRA
                #         debit_codicional = inv_otro.supplier_invoice_number
                #     else: ### LIBRE DE VENTA
                #         invoice_num_name = inv_otro.name.split("/")
                ...
                #         else:
                #             debit_codicional="".join([str(s) for s in inv_otro.name if s.isdigit() ]) 
                #     affected_invoice=inv_otro.invoice_affected
                #     credit_codicional=""
                #     doc_type = 'N/D'

                if inv_otro.supplier_invoice_number:
                    invoice_number = inv_otro.supplier_invoice_number
                elif inv_otro.nro_ctrl:
                    invoice_num_name = inv_otro.name.split("/")
                    if len(invoice_num_name) > 2:
                        invoice_num_ctrl= invoice_num_name[2]## libro de venta
                    else:
                        invoice_num_ctrl="".join([str(s) for s in inv_otro.name if s.isdigit() ])  
                    invoice_number = invoice_num_ctrl
                else:
                    invoice_number = inv_otro.ref
                ...
                values = {
                    ...
                    'type': self.get_transaction_type(fb_id, inv_otro.id),

                    'debit_affected': debit_codicional,
                    'credit_affected': credit_codicional,  #'credit_affected': local_inv_nbr and (cliente or proveedor or False),
                    'ctrl_number': not inv_otro.fiscal_printer and (inv_otro.nro_ctrl if inv_otro.nro_ctrl != 'False' else ''),
                    'affected_invoice':  affected_invoice, #'affected_invoice': local_inv_affected,
                    #'affected_invoice_date': fecha,
                    'partner_name': rp_brw.name or 'N/A',
                    'people_type': people_type,
                    'partner_vat': document_v,
                    'invoice_number':invoice_number if not debit_codicional and not credit_codicional  else ' ',
    ...
    #    ait_obj = self.env['account.invoice.tax']
        ...
        for fbl in self.fbl_ids:
            ...
            if fbl.iwdl_id.invoice_id:
                sign = 1 if fbl.doc_type != 'N/C' else -1
  ...
  #      ait_obj = self.env['account.invoice.tax']
        ...
        for fbl_brw in self.fbl_ids:
            sign = 1 if fbl_brw.doc_type != 'N/C' else -1
            '''
            ...
            if fbl_brw.iwdl_id.invoice_id:
                ...
                for line in fbl_brw.invoice_id.invoice_line_ids:
                    base_amount += fbl_brw.invoice_id.currency_id._convert(line.price_subtotal * sign, fbl_brw.invoice_id.company_currency_id, fbl_brw.invoice_id.company_id, fbl_brw.invoice_id.date)
                    tax_amount += fbl_brw.invoice_id.currency_id._convert((line.price_total-line.price_subtotal) * sign, fbl_brw.invoice_id.company_currency_id, fbl_brw.invoice_id.company_id, fbl_brw.invoice_id.date)
                    # base_amount += self._convert(line.price_subtotal * sign, fbl_brw.invoice_id)
                    # tax_amount += self._convert((line.price_total-line.price_subtotal) * sign, fbl_brw.invoice_id)
 ...
 #       ait_obj = self.env['account.invoice.tax']
        ...
        for fbl in fiscal_book.fbl_ids:
            ...
            if fbl.iwdl_id.invoice_id:
                ...
                for line in fbl.invoice_id.invoice_line_ids:
                    ...
                    if busq:
                        ...
                        line_taxes.update({'fb_id': fb_id,
                                           ...
                                           'base_amount': base,
                                           'tax_amount': amount,
                                        #    'base_amount': self._convert(base, fbl.invoice_id),
                                        #    'tax_amount': self._convert(amount, fbl.invoice_id),
                                           'name': name,
                                           'type': fiscal_book.type

                        })
                    # print(amount_field_data)
                    fbl.write(amount_field_data)
                    if line_taxes:
                        fiscal_taxes.create(line_taxes)
                    else:
                ...
                # if fbl.invoice_id.currency_id != fbl.invoice_id.company_currency_id:
                #     general_base = fbl.vat_general_base *rate
                #     ivaa = general_base * tax.amount / 100
                #     total_ivaa= general_base+ivaa
                # else:
                #     total_ivaa=fbl.invoice_id.company_currency_id.round(fbl.total_with_iva * rate)
                ...
                fbl.write({
                    ...
                    # 'total_with_iva': self._convert(fbl.total_with_iva , fbl.invoice_id),
                    # 'vat_sdcf': self._convert(fbl.vat_sdcf , fbl.invoice_id),
                    # 'vat_exempt': self._convert(fbl.vat_exempt , fbl.invoice_id),
                    # 'vat_general_base': self._convert(fbl.vat_general_base , fbl.invoice_id)
                })
            ...
            if fbl_brw.iwdl_id.invoice_id:
               ...
               for line in fbl_brw.iwdl_id.invoice_id.invoice_line_ids:
                   ...
                   for field_name in field_names:
                        ...
                        # base = self._convert(line.price_subtotal, fbl_brw.iwdl_id.invoice_id)
                        #tax_amount = self._convert(line.price_total - line.price_subtotal, fbl_brw.iwdl_id.invoice_id)
               ...
               fbl_brw.write(data)
                ...
                for line in fbl_brw.invoice_id.invoice_line_ids:
                    ...
                    for field_name in field_names:
                        ...
                        # base = self._convert(line.price_subtotal, fbl_brw.invoice_id)
                        #tax_amount = self._convert(line.price_total - line.price_subtotal, fbl_brw.invoice_id)
    ...
    def get_doc_type(self, inv_id=None, iwdl_id=None, cf_id=None):
        ...
        depending of the invoice type: Debit Note 'N/D', Credit Note 'N/C',
        Invoice 'FACT'.
