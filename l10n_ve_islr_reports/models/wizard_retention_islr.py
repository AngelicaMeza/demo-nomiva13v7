...
class RetentionISLR(models.Model):
    ...
    def generate_retention_islr_xls(self):
        ...
        writer.write_merge(row, row, 2, 2, "FECHA RETENCIÓN", sub_header_style_bold)
        writer.write_merge(row, row, 3,4,  "PROVEEDOR", sub_header_style_bold)
        writer.write_merge(row, row, 5, 5, "DOCUMENTO IDENT/RIF:", sub_header_style_bold)
        writer.write_merge(row, row, 6, 6, "FACTURA:", sub_header_style_bold)
        writer.write_merge(row, row, 7, 7, "CONTROL:", sub_header_style_bold)
        writer.write_merge(row, row, 8, 8, "CONCEPTO", sub_header_style_bold)
        writer.write_merge(row, row, 9, 9, "CODIGO CONCEPTO", sub_header_style_bold)
        writer.write_merge(row, row, 10, 10, "MONTO SUJETO A RETENCIÓN", sub_header_style_bold)
        writer.write_merge(row, row, 11, 11, "TASA PORC", sub_header_style_bold)
        writer.write_merge(row, row, 12, 12, "IMPUESTO RETENIDO", sub_header_style_bold)
        row += 1
        #/////////////////////////////////////////////////////////////////////////////
        ...
        for concept_line in islr_concept_line:
            ...
            fecha_retencion = concept_line.islr_wh_doc_id.date_uid.strftime('%d-%m-%Y')
            ...
            writer.write_merge(row, row, 2, 2,   fecha_retencion, line_content_style_2)
            writer.write_merge(row, row, 3, 4,    concept_line.invoice_id.partner_id.name, line_content_style_2)
            writer.write_merge(row, row, 5, 5,   document, line_content_style_2)
            writer.write_merge(row, row, 6, 6,   concept_line.invoice_id.supplier_invoice_number, line_content_style_2)
            writer.write_merge(row, row, 7, 7,   nro_control, line_content_style_2)
            writer.write_merge(row, row, 8, 8,   concept_line.concept_id.display_name, line_content_style_2)
            writer.write_merge(row, row, 9, 9,  cod_concepto, line_content_style_2)
            writer.write_merge(row, row, 10, 10,  self.separador_cifra(concept_line.base_amount), line_content_style)
            writer.write_merge(row, row, 11, 11,  concept_line.retencion_islr, line_content_style)
            writer.write_merge(row, row, 12, 12, self.separador_cifra(concept_line.amount), line_content_style)
            row += 1
            total_base_amount +=  concept_line.base_amount
            total_amount_ret += concept_line.amount

        writer.write_merge(row, row, 9, 9, "TOTALES", sub_header_style_bold)
        writer.write_merge(row, row, 10, 10, self.separador_cifra(total_base_amount), line_content_style)
        writer.write_merge(row, row, 12, 12, self.separador_cifra(total_amount_ret), line_content_style)
    ...
    def generate_retention_islr_pdf(self):
        ...
        data = {
            'ids': self.ids,
            'model': 'report.l10n_ve_islr_reports.report_retention_islr1',
            'form': {
        ...
        return self.env.ref('l10n_ve_islr_reports.action_report_retention_islr').report_action(self, data=data)
...
class ReportRetentionISLR(models.AbstractModel):
    _name = 'report.l10n_ve_islr_reports.report_retention_islr1'
        ...
        for concept_line in islr_concept_line:
            if concept_line.invoice_id:
                if concept_line.invoice_id.nro_ctrl:
                    ...
                    nro_control = concept_line.invoice_id.nro_ctrl
                fecha_factura = concept_line.invoice_id.date
                #fecha_inicio = fecha_factura.strftime('%d-%m-%Y')
                fecha_retencion = concept_line.islr_wh_doc_id.date_uid
                for cod in concept_line.concept_id.rate_ids:
                    if cod.wh_perc == concept_line.retencion_islr:
                        cod_concepto = cod.code
                if  concept_line.invoice_id.partner_id.company_type == 'person':
