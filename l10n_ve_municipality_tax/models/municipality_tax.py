...
class MUnicipalityTax(models.Model):
    ...
    def get_name(self):
        '''metodo que crea el Nombre del asiento contable si la secuencia no esta creada.'''
        self.ensure_one()
        if not name:
            name = self.env['ir.sequence'].next_by_code('purchase.muni.wh.voucher.number') or '/'
        return name
