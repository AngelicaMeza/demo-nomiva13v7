...
class RepComprobanteIslr(models.AbstractModel):
    ...
    def _get_islr_wh_code_concept(self, concept_line, partner_id):
        people_type = partner_id.people_type_company if partner_id.is_company else partner_id.people_type_individual
        rate = self.env['islr.rates'].search([('concept_id','=',concept_line.concept_id.id),('name','=',people_type.upper())])
        code = rate[0].code if rate else ''
        return code
